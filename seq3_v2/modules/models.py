...
class Seq2Seq2Seq(nn.Module, RecurrentHelper):
    ...
    def __init__(self, n_tokens, **kwargs):
        ...
        self.inp_encoder = SeqReader(self.n_tokens, double=False, **kwargs)
        self.cmp_encoder = SeqReader(self.n_tokens, double=True, **kwargs)

        # backward-compatibility for older version of the project
        kwargs["rnn_size"] = kwargs.get("dec_rnn_size", kwargs.get("rnn_size"))
        enc_size = self.inp_encoder.rnn_size
        self.compressor = AttSeqDecoder(False, self.n_tokens, enc_size, **kwargs)
        self.compressor_for_minor = AttSeqDecoder(False, self.n_tokens, enc_size, **kwargs)
        self.compressor2 = AttSeqDecoder(True, self.n_tokens, enc_size, **kwargs)
        self.compressor2_for_minor = AttSeqDecoder(True, self.n_tokens, enc_size, **kwargs)
        self.decompressor = AttSeqDecoder(True, self.n_tokens, enc_size, **kwargs)
    ...
    def initialize_embeddings(self, embs, trainable=False):
        ...
        self.compressor_for_minor.embed.embedding = embedding_layer
        self.compressor2.embed.embedding = embedding_layer
        self.compressor2_for_minor.embed.embedding = embedding_layer
    ...
    def set_embedding_gradient_mask(self, mask):
        ...
        self.compressor_for_minor.embed.set_grad_mask(mask)
        self.compressor2.embed.set_grad_mask(mask)
        self.compressor2_for_minor.embed.set_grad_mask(mask)
    ...
    def forward(self, inp_src, inp_trg,
        ...
        # --------------------------------------------
        print('shapes:', dists_dec1.shape, dists_dec1.shape)
        abstract2 = torch.cat([dists_dec1, dists_dec1], dim=-1)    #MODIFIED
        cmp_embeddings = self.compressor2.embed.expectation(abstract2)
        ...
        # --------------------------------------------
        dists_dec1_full_info = torch.cat([dists_dec1, dists_dec1_for_minor], dim=-1)
        cmp_embeddings_full_info = self.compressor2.embed.expectation(dists_dec1_full_info)
